...
void Game::handle()
{
    ...
    for(std::size_t i = 0; i < bullets_.size(); )
    {
        ...
        if(map_.collides(b.bounds())) {          // в стену
        ...
        if(hit){
            boom_.play();
            bullets_.clear();        // очистили сразу ВСЕ пули — безопасно
            resetRound();            // респавн танков
            break;                   // выходим из цикла, итераторов больше нет
        } else {
            ++i;                     // пуля жива, двигаемся дальше
        }
    }
...
void Game::spawnBonus()
{
    ...
    std::uniform_int_distribution<int> kind(0,1);          // 0 speed, 1 shield
